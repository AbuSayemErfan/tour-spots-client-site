...
const MyList = () => {
  ...
  useEffect(() => {
    fetch(
      `https://tour-spots-server-side-main.vercel.app/myList/${user?.email}`
    )
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
  ...
  const handleDelete = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://tour-spots-server-side-main.vercel.app/delete/${_id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
  ...
  return (
    <div className="mt-12 mb-24 p-4  lg:p-24  dark:bg-[#f4f3f0] dark:text-gray-800">
      ...
      <Fade direction="left">
        <h1 className="mb-12 text-xl md:text-3xl text-center font-bold">
          My list:{" "}
          <span className="bg-primary px-3 rounded font-normal">
            {details.length}
          </span>
        </h1>
      </Fade>
            ...
            <tbody>
              ...
              {details.map((detail, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <span className="badge badge-ghost badge-sm bg-primary ">
                      {detail.average_cost}
                    </span>
                  </td>
